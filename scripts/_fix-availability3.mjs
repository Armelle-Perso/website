/**
 * Marks "Diptych Canonera" as available.
 *   node scripts/_fix-availability3.mjs
 */
...
async function run() {
  ...
  for (const s of series) {
    ...
    const updated = s.artworks.map(a => {
      if (!a.title) return a
      if (a.title.toLowerCase().includes('cañonera') || a.title.toLowerCase().includes('canonera')) {
        console.log(`  → ${a.title} (in ${s.title}) → available`)
        changed = true
